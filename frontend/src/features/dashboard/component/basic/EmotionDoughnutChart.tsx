import {Center, Paper, Text, Title, Stack, Group, Box, Flex} from '@mantine/core';
              ...
              <Stack gap="xs">
                  {data.map((entry) => (
                      <Flex key={entry.name} gap="xs">
                          ...
                          <Text size="sm" style={{ minWidth: 40 }}>{entry.name}{total > 0 ? Math.round((entry.value / total) * 100) : 0}%</Text>
                      </Flex>
                  ))}
              </Stack>
          </Flex>
